...
LastEditors: iwenli
LastEditTime: 2020-12-16 11:51:14
...
def get_chapter_content(chapter):
    ...
    if (chapter.Status == 0):
        ...
        # 去除特殊字符
        ebook_txts = content.text.replace('\xa0\xa0\xa0\xa0', '').replace(
            chapter.Url, ''
        ).replace(
            '天才一秒记住本站地址：www.zanghaihuatxt.com。笔趣阁手机版阅读网址：wap.zanghaihuatxt.com',
            '')
        return ebook_txts
